...
const GptSearchBar = () => {
  ...
  const handleSearchClick = ()=>{
    ...
    //   });
    //   const getMoviesSuggestionList = getMovieChoices.choices?.[0]?.message?.content.split(",") || ["Pathan", "Jawan", "Fighter", "Housefull", "Gadar"];
    ...
    // main()
    async function main() {
      const getMoviesSuggestionList = [searchText.current.value];
      const promiseArray = getMoviesSuggestionList.map((item)=> searchMoviesTMDB(item));
      const results = await Promise.all(promiseArray);
      dispatch(addGptSearchMovies({movieNames: getMoviesSuggestionList, movieResults:results}));
    }
